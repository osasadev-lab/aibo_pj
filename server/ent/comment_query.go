...
type CommentQuery struct {
	...
	withTask     *TaskQuery
	withUser     *UserQuery
	withMentions *CommentMentionQuery
...
func (_q *CommentQuery) QueryMentions() *CommentMentionQuery {
...
func (_q *CommentQuery) WithMentions(opts ...func(*CommentMentionQuery)) *CommentQuery {
	query := (&CommentMentionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := _q.withMentions; query != nil {
		if err := _q.loadMentions(ctx, query, nodes,
			func(n *Comment) { n.Edges.Mentions = []*CommentMention{} },
			func(n *Comment, e *CommentMention) { n.Edges.Mentions = append(n.Edges.Mentions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadMentions(ctx context.Context, query *CommentMentionQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CommentMention)) error {
	...
	query.Where(predicate.CommentMention(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.MentionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
