...
func (e CommentEdges) MentionsOrErr() ([]*CommentMention, error) {
	if e.loadedTypes[2] {
...
func (*Comment) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case comment.FieldID, comment.FieldTaskID, comment.FieldUserID:
...
// QueryMentions queries the "mentions" edge of the Comment entity.
func (_m *Comment) QueryMentions() *CommentMentionQuery {
	return NewCommentClient(_m.config).QueryMentions(_m)
}
