...
func (_q *WorkspaceQuery) QueryTasks() *TaskQuery {
...
func (_q *WorkspaceQuery) QueryActivityLogs() *ActivityLogQuery {
...
func (_q *WorkspaceQuery) WithTasks(opts ...func(*TaskQuery)) *WorkspaceQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WorkspaceQuery) WithActivityLogs(opts ...func(*ActivityLogQuery)) *WorkspaceQuery {
	query := (&ActivityLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WorkspaceQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *Task)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Workspace)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *WorkspaceQuery) loadActivityLogs(ctx context.Context, query *ActivityLogQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *ActivityLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Workspace)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(activitylog.FieldWorkspaceID)
	}
