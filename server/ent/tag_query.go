...
func (_q *TagQuery) QueryWorkspace() *WorkspaceQuery {
...
func (_q *TagQuery) QueryTasks() *TaskTagQuery {
...
func (_q *TagQuery) WithWorkspace(opts ...func(*WorkspaceQuery)) *TagQuery {
	query := (&WorkspaceClient{config: _q.config}).Query()
...
func (_q *TagQuery) WithTasks(opts ...func(*TaskTagQuery)) *TagQuery {
	query := (&TaskTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withTasks; query != nil {
		if err := _q.loadTasks(ctx, query, nodes,
			func(n *Tag) { n.Edges.Tasks = []*TaskTag{} },
			func(n *Tag, e *TaskTag) { n.Edges.Tasks = append(n.Edges.Tasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadWorkspace(ctx context.Context, query *WorkspaceQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Workspace)) error {
...
func (_q *TagQuery) loadTasks(ctx context.Context, query *TaskTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *TaskTag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tasktag.FieldTagID)
	}
	query.Where(predicate.TaskTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.TasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
