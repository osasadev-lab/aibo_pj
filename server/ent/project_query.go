...
// QueryStatusColumns chains the current query on the "status_columns" edge.
func (_q *ProjectQuery) QueryStatusColumns() *ProjectStatusColumnQuery {
...
func (_q *ProjectQuery) QuerySections() *SectionQuery {
...
// WithStatusColumns tells the query-builder to eager-load the nodes that are connected to
// the "status_columns" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithStatusColumns(opts ...func(*ProjectStatusColumnQuery)) *ProjectQuery {
	query := (&ProjectStatusColumnClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withStatusColumns = query
	return _q
}
...
func (_q *ProjectQuery) WithSections(opts ...func(*SectionQuery)) *ProjectQuery {
	query := (&SectionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withStatusColumns; query != nil {
		if err := _q.loadStatusColumns(ctx, query, nodes,
			func(n *Project) { n.Edges.StatusColumns = []*ProjectStatusColumn{} },
			func(n *Project, e *ProjectStatusColumn) { n.Edges.StatusColumns = append(n.Edges.StatusColumns, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSections; query != nil {
		if err := _q.loadSections(ctx, query, nodes,
			func(n *Project) { n.Edges.Sections = []*Section{} },
			func(n *Project, e *Section) { n.Edges.Sections = append(n.Edges.Sections, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadStatusColumns(ctx context.Context, query *ProjectStatusColumnQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectStatusColumn)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(projectstatuscolumn.FieldProjectID)
	}
	query.Where(predicate.ProjectStatusColumn(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.StatusColumnsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ProjectQuery) loadSections(ctx context.Context, query *SectionQuery, nodes []*Project, init func(*Project), assign func(*Project, *Section)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(section.FieldProjectID)
	}
	query.Where(predicate.Section(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.SectionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
